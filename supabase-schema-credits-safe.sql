-- Enhanced schema with credit and subscription system (SAFE VERSION)
...
-- Users table (extends auth.users) - only if doesn't exist
...
-- Subscriptions table - only if doesn't exist
...
-- Credit balances table - only if doesn't exist
...
-- Workspaces table - only if doesn't exist
...
-- Workspace members table - only if doesn't exist
...
-- Projects table - only if doesn't exist
...
-- Enhanced usage tracking table - only if doesn't exist
...
-- Credit transactions table for detailed tracking - only if doesn't exist
...
-- Row Level Security (RLS) policies - enable only if not already enabled
DO $$ BEGIN
...
BEGIN
  -- Attempt to read current balance; if missing, initialize via monthly reset
  ...
  -- If no row exists yet (existing users prior to trigger), initialize balance
  IF credit_limit IS NULL THEN
    PERFORM reset_monthly_credits(user_id_param);
    -- Re-select after initialization
    IF credit_type_param = 'message' THEN
      SELECT message_credits, message_credits_used 
      INTO credit_limit, current_used
      FROM credit_balances 
  ...
  -- Safety coalesce to avoid null arithmetic
...
-- Index for better performance (only if they don't exist)
CREATE INDEX IF NOT EXISTS idx_credit_balances_user_id ON credit_balances(user_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_id ON subscriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_usage_user_id_created ON usage(user_id, created_at);
...
-- Success message
SELECT 'Credit system schema applied successfully!' as status;
